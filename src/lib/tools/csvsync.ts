// package is dead - so we copied it here

// this error occured: (node:7336) [DEP0128] DeprecationWarning: Invalid 'main' field in '...package.json' of 'csvsync.js
...
// https://github.com/blink-software/csvsync

/**
 * convert javscript object/array into a .csv string
 */
function stringify(data: any, opts: any) {
	opts = opts || {};
	var delimiter = opts.delimiter || ',';
	var quoteAll = opts.quoteAll;

	// iterate rows
	return data.reduce(function(csv: any, row: any) {
		row = row.map(function(field: any) {
			if (isString(field)) {
				// escape " in the field
				if (field.indexOf('"') > -1) {
 ...
 */
function parse(csv: any, opts: any) {
	opts = opts || {};
	var delimiter = opts.delimiter || ',';
	...
	function parse_line(line: any) {
		var row_out: any = opts.returnObject ? {} : [];
